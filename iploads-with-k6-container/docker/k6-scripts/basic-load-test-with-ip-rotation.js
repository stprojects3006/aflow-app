...
const proxy_response_time = new Trend('proxy_response_time');
...
export const options = {
    ...
    thresholds: {
        ...
        ip_rotation_success: ['rate>0.8'],  // IP rotation success rate should be above 80%
    },
};
...
export function setup() {
    console.log(`[VU ${__VU}] Setting up basic load test with IP rotation`);
    ...
    if (IP_ROTATION_ENABLED) {
        ...
        if (proxyConfig) {
            ...
            proxy_response_time.add(endTime - startTime);
...
export default function (data) {
    ...
    const checks = check(response, {
        ...
        'content contains expected text': (r) => r.body.includes('affluenceit') || r.body.includes('Affluence')
    });
    
    // Log request details periodically
    if (__ITER % 10 === 0 && IP_ROTATION_ENABLED && data.proxyConfig) {
        console.log(`[VU ${__VU}] Request ${__ITER} via ${data.proxyConfig.type} proxy: ${data.proxyConfig.proxy}`);
    }
...
export function handleSummary(data) {
    const summary = {
        'basic-load-test-ip-rotation-summary.json': JSON.stringify({
            test_name: 'Basic Load Test with IP Rotation',
