...
## Overview
AFlow is a Spring Boot application with enhanced monitoring and load testing capabilities, including Grafana dashboards and Prometheus metrics.
...
git clone https://github.com/stprojects3006/aflow-app.git
cd aflow-app
...
### Local Deployment (Recommended for EC2)

**⚠️ IMPORTANT: Always use `bash` to run deployment scripts, NOT `sh`**
...
./deploy-local.sh              # Enhanced deployment with better error handling
```

**❌ DON'T DO THIS:**
```bash
sh ./deploy.sh                 # This will fail - sh doesn't support bash features
sudo sh ./deploy.sh            # This will also fail
```

**✅ DO THIS:**
```bash
bash ./deploy.sh               # Use bash explicitly
./deploy.sh                    # Run directly (if executable)
sudo bash ./deploy.sh          # Use bash with sudo if needed
```
...
#### Issue: "Docker is not installed" error
**Cause:** Running script with `sh` instead of `bash`
**Solution:** Use `bash ./deploy.sh` or `./deploy.sh`

#### Issue: Permission denied
**Cause:** Script not executable
**Solution:** Run `chmod +x deploy.sh`

#### Issue: Docker not found in sudo environment
**Cause:** PATH issues with sudo
**Solution:** Use `sudo bash ./deploy.sh` instead of `sudo sh ./deploy.sh`

### Deployment Scripts

- **`deploy.sh`** - Basic deployment script
- **`deploy-local.sh`** - Enhanced deployment with better error handling and dashboard import
- **`docker-compose.yml`** - Local development setup
- **`docker-compose-aws.yml`** - AWS-optimized setup with health checks
...
- **QueueIt Integration**: Load testing and queue management
- **Enhanced Grafana Dashboards**: Comprehensive monitoring dashboards
- **Prometheus Metrics**: Application and system metrics
- **Health Checks**: Container health monitoring
- **Automatic Dashboard Import**: All dashboards imported automatically
...
### Grafana Dashboards
The application includes several enhanced dashboards:
- Application HTTP Traffic
- PetClinic HTTP Traffic
- QueueIt Error Tracking
- QueueIt Integration
- QueueIt Request Volume

### Prometheus Metrics
- Application metrics via Spring Boot Actuator
- Custom QueueIt metrics
